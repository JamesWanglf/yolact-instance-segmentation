...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=1, fps_str=''):
    ...
    print(scores, classes, num_dets_to_consider)
    ...
    def get_color(j, on_gpu=None):
        ...
        if on_gpu is not None and color_idx in color_cache[on_gpu]:
            ...
            if not undo_transform:
                ...
                # color = (color[2], color[1], color[0], 1) # nick add alpha channel
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        # print(f'color: {type(colors)} {masks_color.shape}')
...
def get_result_meta(dets_out, img, h, w, undo_transform=True):
    ...
    for i in range(0, len(classes)):
        ...
        instance['polygons'] = binary_mask_to_polygon(instance_mask, 4)
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    img_numpy = prep_display(preds, frame, None, None, undo_transform=False, class_color=True)
    if img_numpy is None:
        return ''
    h, w, ch = img_numpy.shape
    # Make a True/False mask of pixels whose BGR values sum to more than zero
    alpha = np.sum(img_numpy, axis=-1) > 0

    # # Convert True/False to 0/255 and change type to "uint8" to match "na"
    alpha = np.uint8(alpha * 255)

    img_numpy = np.dstack((img_numpy, alpha))

    # rescaled = cv2.resize(img_numpy, dsize=(200, 200), interpolation=cv2.INTER_CUBIC)
    ret, buffer = cv2.imencode(".png", img_numpy)
