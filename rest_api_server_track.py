...
class DetectAndTrack:
    ...
    def pipeline(self, unit_detections):
        """
        ...
        for trk_idx, det_idx in matched:
            ...
            tmp_trk.no_losses = 0
            #. added by nicky
            track_mat[det_idx] = tmp_trk.tracking_id
...
kalman_tracker = DetectAndTrack(KalmanTracker)
...
def get_result_meta(dets_out, img, h, w, undo_transform=True):
    ...
    for i in range(0, len(classes)):
        ...
        dobject = UnitObject(box = [int(boxes[i][0]),int(boxes[i][1]),int(boxes[i][2]),int(boxes[i][3])], idx=int(classes[i]))
        unitdetections.append(dobject)
        instances.append(instance)
    
    track_list = kalman_tracker.pipeline(unitdetections)
    for j in range(0, len(instances)):
        try:
            instances[j]['tracking_id'] = track_list[j]
        except:
            instances[j]['tracking_id'] = 0
